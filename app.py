...
interval=2 # interval to fetch heart rate for every 2 seconds from arduino
...
def dashboard():
    ...
    while count<6:
        ...
        if line and line!="Test":
            print('Incoming heartrate is '+line)
            if int(line)>120:
                print(line)
                sendSms()
                saveHeartRate(int(line))
            count=count+1

    #print(all_heartrates)
...
def saveHeartRate(heartRate):
    max_id = db.session.query(func.max(HeartRate.heartrateid)).first()[0]
    heartRate=HeartRate(heartrateid = max_id+1,heartbeatvalue=heartRate,userid=int(session['user_id']),createddate=datetime.utcnow())
...
def sendSms():
    ...
    account_sid = 'AC4ad9905e126dd899c9c188ae265232e2'
    auth_token = '4f3bad30b7b5cbb0d4c74e1cddf4aff1'
    twilioClient = Client(account_sid, auth_token)
    loggedInUserContact = Users.query.filter_by(id=int(session['user_id'])).first().emergencycontactno
    message = twilioClient.messages.create(
  from_='+18556271428',
  body='EMERGENCY !!! Heart Beat Exceeded',
  to=loggedInUserContact
    )
